...
Router.post('/add-appointment-staff', async (req, res) => {
    try {
        ...
        const service_appointment = new ServiceAppointment({
            ...
            status,
            time,
            staff_id: await SignupUser.findById(staff_id),
            service_id: await Service.findById(service_id),
            package_id: await Package.findById(package_id),
            appointment_id: await Appointment.findById(appointment_id)
        });
